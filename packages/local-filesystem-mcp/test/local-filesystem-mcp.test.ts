...
`, 'utf8');
  ...
  assert.equal(pagedGlob.result.structuredContent.continuation.offset_semantics.includes('not a materialized-output character offset'), true);
  ...
  assert.equal(snapshotGlob.result.structuredContent.continuation.arguments.snapshot_id, snapshotGlob.result.structuredContent.snapshot_id);
  assert.equal(snapshotGlob.result.structuredContent.continuation.arguments.cache_policy, 'snapshot');
