...
fn run() -> Result<(), String> {
    let projection = projection(env::args().skip(1).collect())?;
    ...
    while let Some(body) = read_message(&mut reader)? {
        ...
        let response = match method {
            "initialize" => json!({"jsonrpc":"2.0","id":id,"result":{"protocolVersion":request.pointer("/params/protocolVersion").and_then(Value::as_str).unwrap_or("2026-04-18"),"capabilities":{"tools":{},"resources":{},"prompts":{},"completions":{},"logging":{}},"serverInfo":{"name":"agent-context-mcp","version":"0.1.0"}}}),
            "tools/list" => json!({"jsonrpc":"2.0","id":id,"result":{"tools":tools}}),
            "notifications/initialized" => continue,
            _ => json!({"jsonrpc":"2.0","id":id,"error":{"code":-32601,"message":format!("agent_context_native_method_not_implemented:{method}")}}),
        };
        write_message(&mut stdout, &response)?;
        stdout.flush().map_err(|error| format!("agent_context_native_flush_failed:{error}"))?;
    }
    Ok(())
}
...
fn read_message(reader: &mut impl BufRead) -> Result<Option<Vec<u8>>, String> {
    ...
    loop {
        ...
        if bytes == 0 {
            return if content_length.is_none() { Ok(None) } else { Err("agent_context_native_unexpected_eof".into()) };
        }
        if line == "\r\n" || line == "\n" { break; }
...
fn projection(args: Vec<String>) -> Result<String, String> {
    let mut projection = "occupant".to_string();
    let mut index = 0;
    while index < args.len() {
        match args[index].as_str() {
            "--tool-projection" => {
                projection = args.get(index + 1).cloned().ok_or("agent_context_native_projection_required")?;
                index += 2;
            }
            "--site-root" | "--site-id" => index += 2,
            value => return Err(format!("agent_context_native_argument_unknown:{value}")),
        }
    }
    if projection != "occupant" && projection != "admin" {
        return Err(format!("agent_context_native_projection_invalid:{projection}"));
    }
    Ok(projection)
}
...
mod tests {
    ...
    fn compiled_catalog_has_unique_tools_for_both_projections() {
        ...
        let occupant = catalog.pointer("/projections/occupant").and_then(Value::as_array).expect("occupant tools");
        let admin = catalog.pointer("/projections/admin").and_then(Value::as_array).expect("admin tools");
        assert!(!occupant.is_empty());
        assert!(admin.len() >= occupant.len());
        for tools in [occupant, admin] {
            let names = tools.iter().map(|tool| {
                assert!(tool.get("inputSchema").is_some(), "every tool must retain its schema");
