...
assert.throws(
  () => parseArgs(['--materialize-all']),
  /registrar_unknown_cli_argument:--materialize-all/,
);
assert.throws(() => parseArgs(['--runtime-profile', 'bun']), /registrar_unknown_cli_argument:--runtime-profile/);
...
try {
  ...
  if (nativeRuntimeArtifactAvailable && nativeSharedSurfaceArtifactAvailable) {
    for (const [surfaceId, args] of [
      ...
      ['nars-session', ['--site-root', '{site_root}']],
      ['quota-meter', ['--site-root', '{site_root}']],
      ['operator-console-overlay', ['--site-root', '{site_root}']],
      ['browser-control', ['--site-root', '{site_root}']],
      ['cloudflare-carrier', ['--site-root', '{site_root}']],
  ...
  assertRuntimeProxy(speechServer, nativeSharedSurfaceEntrypoint, 'narada-mcp-surfaces');
