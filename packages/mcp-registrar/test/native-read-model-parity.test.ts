...
try{
  ...
  for(const carrier of carriers.items){for(const [name,args] of [['registrar_carrier_bind',{carrier_id:carrier.carrier_id,surface_id:'agent-context'}],['registrar_carrier_bind',{carrier_id:carrier.carrier_id,surface_id:'quota-meter'}],['registrar_carrier_unbind',{carrier_id:carrier.carrier_id,surface_id:'agent-context'}]] as const)assert.deepEqual(await rust.callError(name,args),await ts.callError(name,args),`${name} ${carrier.carrier_id} refusal parity`)}
  for(const args of [{target:'all_surfaces_to_carriers',carrier_id:'codex-andrey'},{target:'all_surfaces_to_all_carriers'}]) assert.deepEqual(await rust.callError('registrar_sync',args),await ts.callError('registrar_sync',args),'registrar_sync progressive refusal parity');
  assert.deepEqual(await rust.call('registrar_sync',{target:'all_carriers',surface_id:'quota-meter'}),await ts.call('registrar_sync',{target:'all_carriers',surface_id:'quota-meter'}),'registrar_sync carrier composition parity');
...
function client(executable:string,args:string[],environment:NodeJS.ProcessEnv={}){
  ...
  return {async call(name:string,argumentsValue:unknown){const response=await request(name,argumentsValue);assert.equal(response.error,undefined,response.error?.message??stderr);return response.result.structuredContent},async callError(name:string,argumentsValue:unknown){const response=await request(name,argumentsValue);assert.notEqual(response.error,undefined);return response.error},stop(){child.stdin.end();return new Promise<void>(resolve=>{if(child.exitCode!==null)return resolve();child.once('exit',()=>resolve());setTimeout(()=>{child.kill();resolve()},1000).unref()})}};
  async function request(name:string,argumentsValue:unknown){const requestId=++id;const body=Buffer.from(JSON.stringify({jsonrpc:'2.0',id:requestId,method:'tools/call',params:{name,arguments:argumentsValue}}));child.stdin.write(`Content-Length: ${body.length}\r\n\r\n`);child.stdin.write(body);return waitFor(requestId)}
