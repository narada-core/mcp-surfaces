...
export function surfaceDefinition(): DefinedSurface {
  return defineNativeSurface({
    ...
    projections: [{
      id: 'default',
      transport: { kind: 'stdio', command: 'node', args: ["--allowed-root","{workspace_root}","--allow-command","node","--allow-command","pnpm","--allow-command","npm","--allow-command","python","--allow-prefix","uv run --with sympy python"], env: [] },
