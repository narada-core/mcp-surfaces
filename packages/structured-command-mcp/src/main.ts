...
function buildExecutionStructuredContent(payload: any, { truncated, renderedTextLength, fullTextLength, state: _state }: any) {
  ...
  return {
    ...
    owned_artifact: {
      schema: 'narada.structured_command.owned_execution_artifact.v1',
      artifact_ref: payload.execution_ref,
      owner_surface: 'structured-command',
      result_schema: payload.schema,
      canonical_invocation: { command: payload.command, args: payload.args, working_directory: payload.working_directory },
      source_resolution: payload.command_resolution ?? null,
      streams: { stdout: 'persisted', stderr: 'persisted' },
      reader: { tool: 'structured_command_execution_show', arguments: { execution_ref: payload.execution_ref } },
    },
