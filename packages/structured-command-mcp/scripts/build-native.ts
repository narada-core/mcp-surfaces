...
for (const name of ['main', 'guidance', 'policy', 'command-resolution', 'background-runner']) {
...
const executableName = `narada-structured-command-mcp${process.platform === 'win32' ? '.exe' : ''}`;
const source = join(nativeRoot, 'target', 'release', executableName);

if (!['win32', 'linux', 'darwin'].includes(process.platform)) {
  process.stdout.write(JSON.stringify({
    schema: 'narada.structured_command.native_build.v1',
    status: 'skipped',
...
if (result.status !== 0) throw new Error(`structured_command_native_build_failed:${result.status ?? 'signal'}`);
if (!existsSync(source)) throw new Error(`structured_command_native_artifact_missing:${source}`);
...
if (!executable) throw new Error('structured_command_native_artifact_publication_missing');
process.stdout.write(JSON.stringify({
  schema: 'narada.structured_command.native_build.v1',
  status: 'built',
  executable,
