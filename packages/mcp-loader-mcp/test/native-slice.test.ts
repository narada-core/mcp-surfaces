...
`, 'utf8');
  ...
  const processHandle = spawn(executable, ['--standalone-ambient-attachment', '--child-command', process.execPath, '--allowed-entrypoint-prefix', root, '--attach-timeout-ms', '3000', '--tool-call-timeout-ms', '3000'], { stdio: ['pipe', 'pipe', 'pipe'], windowsHide: true });
  let buffer = '';
