...
fn owned_port_lookup(arguments: &JsonObject, state: &LoaderState) -> Result<Value, Diagnostic> {
    let port = arguments
        .get("port")
        .and_then(Value::as_u64)
    ...
    for line in text.lines().filter(|line| line.contains(&needle)) {
        let Some(pid) = line.split_whitespace().last().and_then(|value| value.parse::<u32>().ok()) else { continue; };
        let Some(connection) = owned.get(&pid) else { continue; };
        owners.push(json!({
            "port":port,
            "pid":pid,
    ...
    Ok(json!({"schema":"narada.mcp_loader.owned_port_lookup.v1","status":"ok","port":port,"owner_count":owners.len(),"owners":owners,"scope":"loader_owned_direct_children_only"}))
}
...
fn compact_input_contract(input_schema: &Value) -> Value {
...
fn inspect_attached_tool(arguments: &JsonObject, state: &LoaderState) -> Result<Value, Diagnostic> {
    let connection = get_connection(arguments, state)?;
...
fn validate_schema_lease(
    ...
    let supplied =
        required_string(arguments, "schema_lease", "schema_lease_required").map_err(|_| {
            Diagnostic::new("schema_lease_required", "schema_lease_or_contract_digest_required").with_details(json!({
                "connection_id": connection_id,
                "tool_name": tool_name,
...
fn call_binding_tool(arguments: &JsonObject, state: &mut LoaderState) -> Result<Value, Diagnostic> {
    ...
    if let Some(value) = arguments.get("tool_schema_digest").or_else(|| arguments.get("tool_contract_digest")) {
...
fn payload_revision_location(site_root: &str, reference: &str) -> Result<String, Diagnostic> {
...
fn verified_payload_revision(path: &str, reference: &str) -> Result<(String, Value), Diagnostic> {
...
mod tests {
    use super::{
        ...
        compact_input_contract, compact_tool_contract, extract_proxy_child_args, list_tools,
        render_result, request_error_details, runtime_freshness, schema_lease_digest,
...
mod modern_protocol_tests {
    ...
    fn payload_ref_is_staged_from_one_admitted_site_into_target_site() {
        let base =
            std::env::temp_dir().join(format!("narada-loader-payload-{}", std::process::id()));
