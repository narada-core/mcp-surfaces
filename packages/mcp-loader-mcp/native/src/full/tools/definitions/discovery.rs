...
pub(crate) fn guidance_definition() -> Value {
    json!({
        ...
        "inputSchema":{"type":"object","properties":{
            ...
            "include_details":{"type":"boolean","default":false,"description":"Include extended workflow guidance instead of the compact actionable summary."},
            "include_runtime_metadata":{"type":"boolean","default":false,"description":"Include loader lifecycle and freshness metadata."}
...
pub(crate) fn list_tools() -> Vec<Value> {
    let mut tools = vec![
        ...
        tool_definition("mcp_loader_list_site_surfaces","List resolvable MCP surfaces declared in a site's local fabric. Returns compact binding IDs and directly usable next calls by default; runtime and command details are opt-in.",json!({"site_root":{"type":"string"},"include_runtime_metadata":{"type":"boolean","default":false},"include_surface_details":{"type":"boolean","default":false,"description":"Include declared command, args, and environment-variable names."}}),&["site_root"],true,false),
        ...
        tool_definition("mcp_loader_inspect_tool","Issue a generation-bound schema lease and an exact-contract summary. include_tool_contract accepts false, compact (default), or verbose.",json!({"connection_id":{"type":"string"},"tool_name":{"type":"string"},"include_tool_contract":{"type":["boolean","string"],"enum":[false,"compact","verbose"],"default":"compact"}}),&["connection_id","tool_name"],true,false),
        tool_definition("mcp_loader_inspect_binding_tool","Resume or open one admitted binding, issue a schema lease, and return an exact-contract summary. include_tool_contract accepts false, compact (default), or verbose.",json!({"site_root":{"type":"string"},"binding_id":{"type":"string"},"surface_id":{"type":"string"},"runtime_kind":{"type":"string"},"tool_name":{"type":"string"},"include_tool_contract":{"type":["boolean","string"],"enum":[false,"compact","verbose"],"default":"compact"}}),&["site_root","binding_id","tool_name"],false,false),
        tool_definition("mcp_loader_inspect_binding_tools","Resume or open one admitted binding once and issue leases for a bounded set of exact tool contracts. include_tool_contract accepts false, compact (default), or verbose.",json!({"site_root":{"type":"string"},"binding_id":{"type":"string"},"surface_id":{"type":"string"},"runtime_kind":{"type":"string"},"tool_names":{"type":"array","minItems":1,"maxItems":20,"uniqueItems":true,"items":{"type":"string","minLength":1}},"include_tool_contract":{"type":["boolean","string"],"enum":[false,"compact","verbose"],"default":"compact"}}),&["site_root","binding_id","tool_names"],false,false),
