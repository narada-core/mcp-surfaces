...
function run(root, requests) {
  ...
  assert.ok(lines.length > 0, 'native restart process produced no response');
...
try {
  ...
  const call = (id, mode) => ({ jsonrpc: '2.0', id, method: 'tools/call', params: { name: 'task_lifecycle_restart', arguments: { mode } } });
  ...
  console.log(JSON.stringify({ schema: 'narada.mcp_lifecycle_native.restart.v1', status: 'passed', checks: ['same_process_refused', 'fresh_process_acknowledged', 'marker_cleared'] }));
} finally {
