...
fn emit_codex(carrier: &CarrierInput) -> Result<Vec<u8>, Failure> {
    let mut out = String::from("# Generated by narada-mcp-materializer. Do not hand-edit.\n\n");
...
fn inline_table(values: &BTreeMap<String, String>) -> Result<String, Failure> {
    Ok(format!(
        "{{{}}}",
        values
            .iter()
            .map(|(key, value)| Ok(format!("{} = {}", toml_key(key), json_string(value)?)))
