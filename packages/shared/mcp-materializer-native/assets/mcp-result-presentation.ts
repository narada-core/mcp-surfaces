...
export function summarizeMcpResult(result: any, fullText: string, modelVisibleText?: string): string {
  const structured = result?.structuredContent;
  const withModelVisibleSize = (summary: string): string => modelVisibleText === undefined
    ? summary
    : `${summary} · model-visible ${counted(modelVisibleText.length, 'character')}`;
  ...
  if (typeof path === 'string' && typeof lines === 'number') {
    if (lines === 0 && typeof structured?.total_lines === 'number') {
      ...
      if (typeof start === 'number' && start > structured.total_lines) {
        return withModelVisibleSize(`range ${range} is past EOF; ${path} has ${structured.total_lines} lines`);
      }
      return withModelVisibleSize(`no lines returned from ${range}; ${path} has ${structured.total_lines} lines`);
    }
    return withModelVisibleSize(`read ${path} (${counted(lines, 'line')})`);
  }
  if (structured?.schema === 'local.filesystem.stat.v1' && typeof path === 'string') {
    ...
    const size = typeof structured?.size === 'number'
      ...
      : '';
    return withModelVisibleSize(`${type} ${path}${size}`);
  }
  if (structured?.schema === 'local.filesystem.str_replace_file.v1' && typeof path === 'string') {
    ...
    return withModelVisibleSize(`replaced ${counted(occurrences, 'occurrence')} in ${path}`);
  }
  if (structured?.schema === 'narada.task.inbox.bridge.v1') {
    ...
    return withModelVisibleSize(`${status} · ${counted(count, 'envelope')}`);
  }
  if (structured?.schema === 'narada.epistemic.issue-tree.resume.v1') {
    ...
    return withModelVisibleSize(`issue tree · ${selectedLabel} · ${counted(open, 'open leaf', 'open leaves')}${version != null ? ` · version ${version}` : ''}`);
  }
  ...
  if (schema === 'narada.mcp_loader.result_page.v1') {
    return withModelVisibleSize(`MCP loader result page${typeof status === 'string' ? ` · ${status}` : ''} · ${counted(chars, 'character')}`);
  }
  const identity = [schema, status].filter((value) => typeof value === 'string').join(': ');
  return withModelVisibleSize(`${identity || 'MCP result'} (${counted(chars, 'character')})`);
}
