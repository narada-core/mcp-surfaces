...
fn capability_snapshot(
    ...
    json!({
        ...
        "workflow_primitives":{"text_file_lifecycle":{"tool":"bounded_shell_command","operations":["create","read_verify","remove","confirm_absent"],"encoding_must_be_explicit":true,"windows_recipe":"use one literal-path PowerShell cmdlet invocation per operation: Set-Content, Get-Content, Remove-Item, then Test-Path; do not use .NET method invocation under ConstrainedLanguage"}},
...
mod tests {
    ...
    fn capability_snapshot_reports_effective_write_posture() {
        ...
        assert_eq!(
            ...
            "use one literal-path PowerShell cmdlet invocation per operation: Set-Content, Get-Content, Remove-Item, then Test-Path; do not use .NET method invocation under ConstrainedLanguage"
        );
