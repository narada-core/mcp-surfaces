...
fn mutation_schema(name: &str) -> Value {
    ...
    let (properties, required): (Map<String, Value>, Vec<&str>) = match name {
        ...
        "sop_action_resolve" => (json!({"action_id":string(),"completion_key":string(),"outcome":{"type":"string","enum":["completed","failed"]},"operation_ref":string(),"result":object(),"result_ref":object(),"error_message":{"type":"string"},"principal":string()}).as_object().unwrap().clone(), vec!["action_id","completion_key","outcome","principal"]),
