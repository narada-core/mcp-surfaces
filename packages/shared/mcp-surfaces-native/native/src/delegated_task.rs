...
impl Drop for TaskLock { fn drop(&mut self) { let _=self.0.unlock(); } }
fn lock_task(root:&Path,id:&str)->Result<TaskLock,Value>{let path=task_path(root,id)?.with_file_name("mutation.lock");fs::create_dir_all(path.parent().unwrap()).map_err(|_|error("delegated_task_lock_failed","delegated_task_lock_failed"))?;let file=OpenOptions::new().create(true).read(true).write(true).open(path).map_err(|_|error("delegated_task_lock_failed","delegated_task_lock_failed"))?;file.lock_exclusive().map_err(|_|error("delegated_task_lock_failed","delegated_task_lock_failed"))?;Ok(TaskLock(file))}
...
fn task_advance(args:&Map<String,Value>,root:&Path)->Result<Value,Value>{ let id=task_id(args)?; let _lock=lock_task(root,&id)?; let task=advance_value(read_task(root,&id)?,root)?; Ok(json!({"schema":"narada.delegated_task.advance.v1","status":"ok","task_id":id,"task_status":task["status"],"task":compact_task(&task)})) }
fn step_status<'a>(task:&'a Value,id:&str)->Option<&'a str>{task.pointer(&format!("/result/step_states/{id}/status")).and_then(Value::as_str)}
fn ready_step_ids(task:&Value)->Vec<String>{
    task.pointer("/workflow/steps").and_then(Value::as_array).into_iter().flatten().filter_map(|step|{
        let id=step.get("id").and_then(Value::as_str)?; if step_status(task,id)!=Some("pending"){return None}
        let ready=step.get("depends_on").and_then(Value::as_array).map(|dependencies|dependencies.iter().filter_map(Value::as_str).all(|dependency|matches!(step_status(task,dependency),Some("completed"|"skipped")))).unwrap_or(true);
        ready.then(||id.to_string())
    }).collect()
}
fn advance_value(mut task:Value,root:&Path)->Result<Value,Value>{
    ...
    let step_ids=task.pointer("/workflow/steps").and_then(Value::as_array).into_iter().flatten().filter_map(|step|step.get("id").and_then(Value::as_str).map(str::to_string)).collect::<Vec<_>>();
    for step_id in &step_ids {
        if step_status(&task,step_id)!=Some("running"){continue}
        let run_id=task.pointer(&format!("/result/step_states/{step_id}/current_run_id")).and_then(Value::as_str).unwrap_or_default().to_string();
        let status=crate::worker_delegation::call_tool("worker_run_status",json!({"run_id":run_id}).as_object().unwrap(),root,&[root.to_path_buf()])?; let worker=status.pointer("/run/status").and_then(Value::as_str).unwrap_or("running").to_string();
        if worker=="completed"{task["result"]["step_states"][step_id]["status"]=json!("completed");task["result"]["step_states"][step_id]["finished_at"]=json!(now());append_event(root,&id,"worker_completed",json!({"step_id":step_id,"run_id":run_id}))?;} else if matches!(worker.as_str(),"failed"|"cancelled"|"completed_with_errors"){task["status"]=json!("failed");task["result"]["step_states"][step_id]["status"]=json!("failed");task["result"]["acceptance_verdict"]=json!("failed");append_event(root,&id,"task_failed",json!({"step_id":step_id,"run_id":run_id,"worker_status":worker}))?;}
    }
    if task.get("status").and_then(Value::as_str)!=Some("failed") {
        let ready=ready_step_ids(&task); let steps=task.pointer("/workflow/steps").and_then(Value::as_array).cloned().unwrap_or_default();
        for step_id in ready { let Some(step)=steps.iter().find(|step|step.get("id").and_then(Value::as_str)==Some(step_id.as_str())) else {continue}; let kind=step.get("kind").and_then(Value::as_str).unwrap_or("worker"); if kind=="gate"{continue}
            let instruction=step.get("instruction").and_then(Value::as_str).or_else(||task.get("objective").and_then(Value::as_str)).unwrap_or_default(); let constraints=step.get("constraints").cloned().unwrap_or_else(||task.get("constraints").cloned().unwrap_or_else(||json!({}))); let worker_args=json!({"intent":{"instruction":instruction},"constraints":constraints});
            let run=crate::worker_delegation::call_tool("worker_run",worker_args.as_object().unwrap(),root,&[root.to_path_buf()])?; let run_id=run.get("run_id").and_then(Value::as_str).unwrap_or_default(); task["status"]=json!("running"); task["result"]["step_states"][&step_id]["status"]=json!("running"); task["result"]["step_states"][&step_id]["current_run_id"]=json!(run_id); task["result"]["step_states"][&step_id]["run_ids"]=json!([run_id]); task["result"]["step_states"][&step_id]["started_at"]=json!(now()); if let Some(refs)=task["result"]["worker_refs"].as_array_mut(){refs.push(json!({"step_id":step_id,"step_kind":kind,"run_id":run_id,"status":"running"}));} append_event(root,&id,"worker_started",json!({"step_id":step_id,"run_id":run_id}))?;
        }
        if !step_ids.is_empty() && step_ids.iter().all(|step_id|matches!(step_status(&task,step_id),Some("completed"|"skipped"))){task["status"]=json!("completed");task["result"]["acceptance_verdict"]=json!("passed");append_event(root,&id,"task_completed",json!({}))?;}
    }
    task["updated_at"]=json!(now()); write_task(root,&task)?; Ok(task)
}
fn task_cancel(args:&Map<String,Value>,root:&Path,takeover:bool)->Result<Value,Value>{let id=task_id(args)?;let _lock=lock_task(root,&id)?;let mut task=read_task(root,&id)?;if matches!(task.get("status").and_then(Value::as_str),Some("completed"|"failed"|"cancelled")){return Err(error("delegated_task_terminal_status","delegated_task_terminal_status"));}task["status"]=json!("cancelled");task["updated_at"]=json!(now());let kind=if takeover{"task_parent_takeover"}else{"task_cancelled"};let detail=if takeover{json!({"parent_task_id":args.get("parent_task_id"),"reason":args.get("reason")})}else{json!({"reason":args.get("reason")})};task["result"][if takeover{"parent_takeover"}else{"cancellation"}]=detail.clone();write_task(root,&task)?;let event=append_event(root,&id,kind,detail)?;Ok(json!({"schema":if takeover{"narada.delegated_task.parent_takeover.v1"}else{"narada.delegated_task.cancel.v1"},"status":if takeover{"parent_takeover_recorded"}else{"cancelled"},"task_id":id,"task_status":"cancelled","event":event}))}
