...
pub fn list_tools() -> Vec<Value> {
    vec![
        ...
        tool("surface_feedback_submit", "Submit feedback through the owning surface-feedback authority. For cross-site observations, omit submitter_site_id and submitter_principal so the server binds authority; put the observed Site and surface in non-authoritative details.", submit_schema(), false),
...
fn submit_schema() -> Value { json!({"type":"object","properties":{"surface_id":{"type":"string","minLength":1,"description":"Observed surface. This is provenance context and does not assert submitter authority."},"submitter_site_id":{"type":"string","minLength":1,"description":"Optional authority assertion that must equal server-bound NARADA_SITE_ID. Omit for ordinary and cross-site submission."},"submitter_principal":{"type":"string","minLength":1,"description":"Optional authority assertion that must equal server-bound NARADA_AGENT_ID. Omit for ordinary and cross-site submission."},"kind":{"type":"string","enum":FEEDBACK_KINDS},"summary":{"type":"string","minLength":1},"details":{"type":"string","description":"Feedback detail; include the observed Site/surface here as non-authoritative provenance when reporting cross-site behavior."},"idempotency_key":{"type":"string","minLength":1,"description":"Stable retry key; reuse with different content is refused."}},"required":["surface_id","kind","summary"],"additionalProperties":false}) }
...
fn guidance(args: &Map<String, Value>) -> Value { json!({"schema":"narada.mcp_surface.guidance.v0","status":"ok","surface_id":"surface-feedback","guidance_tool":"surface_feedback_guidance","purpose":"Inspect bounded feedback evidence with explicit read scope.","requested":{"workflow":args.get("workflow").cloned().unwrap_or(Value::Null),"tool":args.get("tool").cloned().unwrap_or(Value::Null)},"first_use":["Call surface_feedback_doctor first and inspect capabilities.read_scopes.","For ordinary or cross-site submission, omit submitter_site_id and submitter_principal; the server binds both authority values.","Carry an observed Site and surface in surface_id/details as non-authoritative provenance, never as an authority assertion.","Use all_authorized for the canonical local store or store_reconciliation for explicit reconciliation work.","Use list or actionable_queue for bounded discovery, then show before mutation.","Task conversion remains owner-authorized."],"cross_site_submission":{"canonical_route":"Call the canonical surface_feedback_submit binding without submitter_site_id or submitter_principal.","authority_binding":"The server records its bound NARADA_SITE_ID and NARADA_AGENT_ID.","observed_context":"Put the observed surface in surface_id and the observed Site/context in details; these fields do not grant authority."},"read_scope_summary":{"available":["all_authorized","store_reconciliation"],"server_authority_required":["authority_visible","owned_surfaces","authority_site_submissions"]},"boundaries":["The authoritative store is an append-only event ledger under <feedback_root>/ledger; the SQLite projection under .ai/feedback is disposable and rebuilt from the ledger on every read.","The legacy .feedback/surface-feedback.db store is migrated once into the ledger and never written again.","Task creation is delegated to the task-lifecycle authority adapter; Surface Feedback does not own tasks.","Authority and provenance scopes remain server-bound."]}) }
