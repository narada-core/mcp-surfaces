...
| site-loop | in progress | Native durable attention/control authority and allowlisted test execution implemented; test output is bounded and terminal UI inheritance removed. Native proof persistence/execution admits success only after both the carrier terminal receipt and a task-lifecycle report bearing the exact directive id. Native status resolves resident launch/heartbeat/retirement; a no-console Rust host owns the long-lived runtime control channel. Native recovery requires a different live replacement carrier and repeats the correlated proof. Native run-once has an honest bounded dry-run plan, preserves the production scheduler boundary over MCP, admits only configured synchronous test authority, records runs, dispatches admitted resident directives, and reconciles exact outcomes. Scheduled SOPs now compose the Rust SOP authority with deterministic occurrence keys; materializable inbox envelopes compose the Rust inbox and lifecycle authorities via immutable payload refs, durable task creation, and source acknowledgement—no duplicated task persistence. Remaining: production supervisor entrypoint and complete stdio matrix. | focused Rust site-loop 5/5 including SOP occurrence replay, inbox→immutable payload→task bridge, proof transitions, dispatch/reconciliation, run boundary, live/retired projection; resident-host 1/1 | `61d48e0`, `7b2c1fe`, `e9e34e9`, `d88d8a6`, `9f94f46`, `35c67f5`, `18760a3`, `afaaa08`, current thematic commit | pending |
