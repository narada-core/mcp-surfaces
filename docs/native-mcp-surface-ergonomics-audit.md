...
| site-loop | in progress | Native durable attention/control authority and allowlisted test execution implemented; test output is bounded and terminal UI inheritance removed. Native proof persistence/execution admits success only after both the carrier terminal receipt and a task-lifecycle report bearing the exact directive id. Native status resolves resident launch/heartbeat/retirement; a no-console Rust host owns the long-lived runtime control channel. Native recovery requires a different live replacement carrier and repeats the correlated proof. Native run-once records runs, dispatches admitted resident directives, reconciles exact outcomes, schedules SOPs with deterministic occurrence keys, and bridges inbox work through immutable lifecycle payloads. MCP preserves the production scheduler boundary; a strict native supervisor entrypoint now owns repeated production cycles and durable bounded heartbeat records, while dry-run supervisor proof is side-effect free. Remaining: wire the scheduler/default launch contract to that entrypoint and complete the stdio matrix. | focused Rust site-loop 5/5 including SOP/inbox composition, proof, dispatch/reconciliation, run/supervisor boundaries and heartbeat; resident-host 1/1 | `61d48e0`, `7b2c1fe`, `e9e34e9`, `d88d8a6`, `9f94f46`, `35c67f5`, `18760a3`, `afaaa08`, `c52edf5`, current thematic commit | pending |
