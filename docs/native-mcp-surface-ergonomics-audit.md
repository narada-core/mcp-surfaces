...
| site-lifecycle | in progress | Added the four missing operator-surface authority tools over Narada-compatible site-local identity and runtime-binding artifacts; explicit execute/authority gate, target-evidence postcondition, replay, and conflict refusal. Replaced read placeholders with canonical registry list/show, the complete lifecycle-kind catalog and preflight, bounded site-local relation list/validation, and mutation-locus preflight with root confinement. Restored all five native creation presets and faithful bounded greenfield dry-run planning/refusals; the read-only MCP planner refuses hidden `output_plan` writes. Discovery mutation, doctor/init, and dependency synchronization remain. | focused Rust read/authority 2/2 plus simple-surface coverage; real native stdio covers operator authority, presets/plans and refusal paths, registry reads, all lifecycle kinds, ready/blocked preflight, relation validation, mutation-locus classification, confinement, replay/conflict, readback, and artifacts | `64155c7`, `0170e73`, `456bd88`, `1b0d962` | pending |
