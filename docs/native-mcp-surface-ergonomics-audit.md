...
| site-loop | in progress | Native durable attention/control authority and allowlisted test execution implemented; test output is bounded and terminal UI inheritance removed. Added bounded, atomic native resident-proof request persistence, active-request retry reuse, and durable proof status. Native status/health/readiness/coherence now resolve sparse-config default and external session roots, bounded launch results, heartbeat freshness, and retirement rather than returning placeholders. Synchronous proof execution, resident launch, recovery drill, and loop execution still require the native resident coordinator now being ported. | focused Rust 4/4 including proof start/retry/status persistence and live/retired resident projections; real native stdio config/docs/test-list/status/configured-test execution | `61d48e0`, `7b2c1fe`, `e9e34e9`, current thematic commit | pending |
