...
| site-loop | in progress | Native durable attention/control authority and allowlisted test execution implemented; test output is bounded and terminal UI inheritance removed. Added bounded, atomic native resident-proof request persistence, active-request retry reuse, and durable proof status. Native status/health/readiness/coherence resolve sparse-config default and external session roots, bounded launch results, heartbeat freshness, and retirement. Added a native no-console resident host that keeps the Rust Agent Runtime Server control channel alive, forwards only complete bounded JSONL control chunks, records host/runtime identity, and stops retired carriers. Native proof processing launches/reuses that host, delivers an idempotent system directive over its durable control channel, and admits success only after both the carrier terminal receipt and a task-lifecycle report bearing the exact directive id are observed. Native recovery now durably retires the current carrier, requires a different replacement carrier to become live, and accepts the drill only when that replacement passes the same correlated production proof. Loop execution remains in progress. | focused Rust site-loop 4/4 including queued/running/passed proof transitions, exact directive/report correlation, live/retired projection, and retirement persistence; resident-host 1/1 | `61d48e0`, `7b2c1fe`, `e9e34e9`, `d88d8a6`, `9f94f46`, `35c67f5`, current thematic commit | pending |
