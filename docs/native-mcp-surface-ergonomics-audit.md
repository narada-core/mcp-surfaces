...
| site-loop | in progress | Native durable attention/control authority and allowlisted test execution implemented; test output is bounded and terminal UI inheritance removed. Added bounded, atomic native resident-proof request persistence, active-request retry reuse, and durable proof status. Synchronous proof execution, recovery drill, and loop execution still require the native resident coordinator now being ported. | focused Rust 4/4 including proof start/retry/status persistence; real native stdio config/docs/test-list/status/configured-test execution | `61d48e0`, `7b2c1fe`, current thematic commit | pending |
