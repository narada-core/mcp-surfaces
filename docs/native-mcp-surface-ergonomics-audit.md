...
| site-loop | in progress | Native durable attention/control authority and allowlisted test execution implemented; test output is bounded and terminal UI inheritance removed. Added bounded, atomic native resident-proof request persistence, active-request retry reuse, and durable proof status. Native status/health/readiness/coherence resolve sparse-config default and external session roots, bounded launch results, heartbeat freshness, and retirement. Added a native no-console resident host that keeps the Rust Agent Runtime Server control channel alive and forwards bounded durable control frames. Native proof processing admits success only after both the carrier terminal receipt and a task-lifecycle report bearing the exact directive id. Native recovery retires the current carrier, requires a different live replacement, and runs the correlated proof. Native run-once now has an honest bounded dry-run plan, preserves the production scheduler boundary over MCP, admits only explicitly configured synchronous test authority, durably records runs, dispatches admitted resident directives through the native control host, and reconciles exact terminal/report outcomes. Remaining: scheduled-SOP/inbox phase parity, production supervisor entrypoint, and complete stdio matrix. | focused Rust site-loop 4/4 including proof transitions, dispatch/reconciliation, run dry-run boundary, live/retired projection, retirement persistence; resident-host 1/1 | `61d48e0`, `7b2c1fe`, `e9e34e9`, `d88d8a6`, `9f94f46`, `35c67f5`, `18760a3`, current thematic commit | pending |
