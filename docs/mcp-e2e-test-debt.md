...
| --- | --- | --- | --- |
| `delegated-task` | [`native/src/delegated_task.rs`](../packages/shared/mcp-surfaces-native/native/src/delegated_task.rs) | Native Rust task validation, persistence, lifecycle, and worker-handoff authority | External provider behavior and production worker authority are separate |
| `worker-delegation` | [`native/src/worker_delegation.rs`](../packages/shared/mcp-surfaces-native/native/src/worker_delegation.rs) | Native Rust policy, cognition, worker-run, and bounded artifact authority; no TypeScript implementation remains | External provider behavior and production worker authority are separate |
| `surface-feedback` | [`native/src/surface_feedback.rs`](../packages/shared/mcp-surfaces-native/native/src/surface_feedback.rs) | Native Rust feedback intake, routing, and task-handoff authority; no TypeScript implementation remains | External User Site authority remains separate |
...
| P0 | `worker-delegation` | Native Rust worker authority | [`native/src/worker_delegation.rs`](../packages/shared/mcp-surfaces-native/native/src/worker_delegation.rs) contains bounded policy, cognition, run, and result tests; real external-provider authority is not asserted here | native unit/protocol coverage | not_run |
| P1 | `local-filesystem-mcp` | Real governed filesystem child | [`native/tests/protocol_equivalence.rs`](../packages/local-filesystem-mcp/native/tests/protocol_equivalence.rs) crosses the packaged Rust child over MCP and proves tools/list, bounded reads/search, write-mode refusal, and match-all refusal; shared native filesystem tests cover mutation, patch, and recovery behavior | complete | passed |
| P1 | `structured-command-mcp` | Real command policy boundary | [`native/tests/protocol_equivalence.rs`](../packages/structured-command-mcp/native/tests/protocol_equivalence.rs) crosses the packaged Rust child over MCP and proves argv execution, bounded output paging, timeout, refusal classification, input refs, and detached execution | complete | passed |
...
| P1 | `surface-feedback` | Real feedback-to-task fabric | The TypeScript implementation and its site-fabric e2e were removed; [`test/clean-room-site-journey.e2e.test.ts`](../test/clean-room-site-journey.e2e.test.ts) submits and converts feedback through the native `narada-mcp-surfaces` surface | complete | passed |
