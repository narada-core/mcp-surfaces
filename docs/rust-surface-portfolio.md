...
| `mailbox` | Rust-only authority | Rust owns confined and bounded projection reads, finite Graph synchronization, leases/cursors, first-observation reconciliation, mechanical admission, and durable outbox consumers/receipts. Every runtime profile resolves to Rust; Node and Bun are unavailable. |
...
| `surface-feedback` | Rust-native only | The native shared surface owns the bounded feedback contract and exposes routing/cross-site authority boundaries explicitly. The TypeScript rollback implementation was removed; no Node/Bun rollback remains. |
