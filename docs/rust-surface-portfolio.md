...
| `scheduler` | Node operational default | The Rust shared surface currently proves only the scheduler authority boundary and reports `authority_boundary`; the native profile therefore selects Node until Rust owns the operational activation, outbox, and Task Scheduler paths and passes the same live gates. |
